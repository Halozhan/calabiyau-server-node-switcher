def get_latency_stats(region: str, server_addr: str, window: int = 30, db_path: str = "latency.db"):
    """
...
def insert_latency(region: str, server_addr: str, latency: float, success: int, db_path: str = "latency.db"):
    conn = sqlite3.connect(db_path)
    c = conn.cursor()
    c.execute(
...
def get_latency(region: Optional[str] = None, db_path: str = "latency.db"):
    ...
    c = conn.cursor()
    if region:
        c.execute("SELECT * FROM latency WHERE region = ? ORDER BY timestamp DESC", (region,))
    else:
